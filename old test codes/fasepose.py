    ...
    if bbox_ is not None:
        for bbox, landmarks, prob in zip(bbox_, landmarks_, prob_):
            if prob > 0.9:
                ...
                print('The detected face has a probability less than the detection threshold')
    else:
...
def predFacePoseApp(path, url):
    ...
    if path is not None:
        ...
        while True:
            ...
            if landmarks_ is not None and angle_R_List is not None and angle_L_List is not None and predLabelList is not None:
